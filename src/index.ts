...
let regex = new RegExp(
  `"\\[\\d{2}:\\d{2}:\\d{2}\\] Completion Time|.*\\breceived\\b`
);
let chatboxInterval;
let timerAnim;
...
function closeSettings() {
  const settingsElements: any = document.querySelectorAll(
    ".modal-content td:nth-child(even)"
  );
  for (const {
    children: [setting],
  } of settingsElements) {
...
function setError(message) {
  ...
  if (
    !window.alt1 ||
    !window.alt1?.permissionPixel ||
    !window.alt1?.permissionOverlay
  ) {
...
function formatTime(value) {
  ...
  if (hours < 1) {
    return `${minutesS}:${sec}.<span class="miliseconds">${
      mil ? mil : "00"
    }</span>`;
  }
  return `${hoursS}:${minutesS}:${sec}.<span class="miliseconds">${
    mil ? mil : "00"
  }</span>`;
}
...
function capture() {
  ...
  reader.readargs = {
    colors: [
      ...
      a1lib.mixColor(45, 185, 20), // Completion Time
      a1lib.mixColor(30, 255, 0), // Recieved Drop
  ...
  function readChatbox() {
    ...
    const timestamp = killComplete[0].match(
